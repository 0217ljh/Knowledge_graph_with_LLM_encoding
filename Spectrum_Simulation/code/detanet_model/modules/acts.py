...
class ParametricReLU(nn.Module):
    '''Parametric ReLU activation function with learnable parameters'''
    def __init__(self,num_features,inital_alpha=1.00,inital_beta=1.702):
    ...
    def forward(self,x):
        return x*self.alpha*torch.relu(self.beta*x)

# 用法示例
# num_features = 128  # 设定特征数量
# relu_layer = ParametricReLU(num_features)
...
def activations(type,num_features=40):
    '''get activations'''
    act = {'swish': Swish(num_features=num_features),
       ...
       'shiftedsoftplus': ShiftedSoftplus(),
        ...
        'silu':nn.SiLU(),
        #'relu':nn.ReLU(),
        'relu':ParametricReLU(num_features=num_features),
        'sigmoid':nn.Sigmoid(),
        'tanh':nn.Tanh()
        }
    return act[type]