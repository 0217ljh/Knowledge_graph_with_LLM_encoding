...
class DetaNet(nn.Module):
    ...
    device: device type, either torch.device('cuda') represents gpu or torch.device('cpu')  represents cpu
        ...
        self.rc=rc
        self.grad=grad_type
        # self.Train_mijs=Train_mijs
        # self.Train_mijs2=Train_mijs2
        # self.Val_mijs=Val_mijs
        # self.Val_mijs2=Val_mijs2
    ...
    def grad_dipole(self, dipole, pos):
        '''Calculating the derivative of the dipole moment with respect to the coordinates
        require out_type='dipole' and grad_type='dipole' and scalar_outsize=1 and irreps_out='1o'
        '''
        dedipole = torch.zeros(size=(pos.shape[0], 3, 3), device=pos.device)
        
        for i in range(0, 3):
            # Correct grad_outputs to match the shape of outputs (a scalar)
            grad_outputs = torch.tensor(0.1, device=pos.device)
            
            # Compute gradient
            dedipole[:, :, i] = -grad(
                outputs=[dipole[:, i].sum()],
                inputs=[pos],
        ...
        dedipole = torch.clamp(dedipole, min=-1e2, max=1e2)
    ...
    def forward(self,
        ...
        '''
        # if is_training:
        #     mijs = self.Train_mijs
        #     mijs2 = self.Train_mijs2
        # else:
        #     mijs = self.Val_mijs
        #     mijs2 = self.Val_mijs2
        
        # batch_indices = batch.unique()  # 获取当前批次的唯一索引

        # # 获取当前批次的数据数量
        # num_samples = len(batch_indices)

        # # 从整个mijs和mijs2中提取对应的值
        # mijs = [mijs[i.item()] for i in batch_indices]
        # mijs2 = [mijs2[i.item()] for i in batch_indices]
        # mijs=torch.stack(mijs)
        # mijs2=torch.stack(mijs2)
        ...
        # for block in self.blocks:
        #     S,T = block(S=S, T=T, sh=sh, rbf=rbf, index=edge_index,data=data)
