...
def gen_graph(graphs: list[MSAPygDataset,], 
        ...
        category_desc = pd.read_csv(
                    os.path.join('task_data\graph\Task3-Judgment\Small_Molecules\FS_label.csv'), 
                                sep=","
                ).values
        ordered_desc = []
    
        label_name = ['No(is Non-halogen compounds)','Yes(is Halogen compounds)']   # index used to get label 
        ...
        # prompt_text = [
        #     "prompt node. Based on the molecular graph and spectral data, determine whether the molecule contains the 'Phenyl' group. Provide evidence by matching spectral peaks in the range of [1600, 1500] nm and explain how the knowledge graph supports this conclusion. Using the given spectrum and molecular graph, identify if the molecule contains a 'Phenyl' group. Justify your classification with specific spectral peak evidence and the relationships between the spectral peaks and functional groups in the knowledge graph."] 
        #  # 详细版prompt（增加了具体的peak范围，以及结合引导任务）

        
        # prompt_text = [
        #             "prompt node. This is a few-shot classification task. Determine whether the given query molecule belongs to the class of support molecules based on its spectral peaks. Here are the support molecules with their spectral peaks: Molecule 1: [1600, 1550] nm, Molecule 2: [1700, 1650] nm. Given the query molecule's spectrum, does it match any of these support classes?",
        #             "Classify the molecule based on its spectral peaks. Consider the given few-shot examples of molecules and their respective peak data. Using this information, decide whether the query molecule shares similarities with the support molecules in terms of its spectral characteristics."
...
class Spec_Graphjdm_Dataset_FS(BasePygDataset):
    def gen_data(self):
