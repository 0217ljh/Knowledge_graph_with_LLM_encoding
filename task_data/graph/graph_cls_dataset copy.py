...
def gen_graph(graphs: list[MSAPygDataset,], 
        ...
        category_desc = pd.read_csv(
                    os.path.join('task_data\graph\Task4-Classification\Small_Molecules\categories_aromatic.csv'), 
                                sep=","
                ).values
        ordered_desc = []
        ...
        label_name = ['Non-aromatic compound','Aromatic compound'] # Index order of label
...
class Spec_Graphcls_Dataset(BasePygDataset):
    def gen_data(self):
