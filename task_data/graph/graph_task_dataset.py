...
class Spec_cla_Dataset(BasePygDataset):
    def gen_data(self):
        with open('./Datasets/Data_for_different_Task/4/FG_graph.pkl', 'rb') as f: # 加载处理后的化学知识数据
            FG_graph = pickle.load(f)
        with open('./Datasets/Data_for_different_Task/4/Spectrum_graph.pkl', 'rb') as f: # 加载处理后的光谱数据
            Load_spec = pickle.load(f)
        datasets = (FG_graph, Load_spec)
        pyg_graph, texts, split = self.gen_graph(get_local_text(datasets))
        return [d for d in pyg_graph], texts, split

    def gen_graph(self,graphs: list[MSAPygDataset,], 
        ...
        label_name = ['Aromatic compound','Non-aromatic compound']
        ...
        "prompt edge. edge for support graph that is an example", ] # 边标签文本

        prompt_text = ["prompt node. node classification on the sample's category based on spectral information and knowledge graph",
        "prompt node. few shot task node for graph classification that decides whether the query molecule belongs to "
        "the class of support molecules.", ]    # 提问prompt文本
        ...
        ret = (data, [u_node_texts_lst, u_edge_texts_lst, u_y_texts_index_lst, prompt_edge_text, prompt_text, ],
...
class Spec_jud_Dataset(Spec_cla_Dataset):
    def gen_data(self):
        with open('./Datasets/Data_for_different_Task/3/FG_graph.pkl', 'rb') as f: # 加载处理后的化学知识数据
            FG_graph = pickle.load(f)
        with open('./Datasets/Data_for_different_Task/3/Spectrum_graph.pkl', 'rb') as f: # 加载处理后的光谱数据
            Load_spec = pickle.load(f)
        datasets = (FG_graph, Load_spec)
        pyg_graph, texts, split = self.gen_graph(get_local_text(datasets))
        return [d for d in pyg_graph], texts, split