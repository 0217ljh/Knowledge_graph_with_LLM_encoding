...
def gen_graph(graphs: list[MSAPygDataset,], 
    ...
    category_desc = pd.read_csv(
                os.path.join("task_data\\node\\Task1_Classification\\FS_label.csv"), 
                            sep=","
            ).values
    ordered_desc = []
    label_name = {'Amines',"Phenyl",'Unknown'}# Index order of label
    ...
    # 读取分子表
    df = pd.read_excel('Database\Task_Set\spect_Task1_fs.xlsx')
    ...
    for num, g in enumerate(graphs):
        ...
        for node_name, node_attr in strength_nodes:
            ...
            if strength_match:
                ...
                print(node_attr['label'],node_label_id)
...
class Spec_Node_Dataset_FS(BasePygDataset):
    def gen_data(self):
        #task_index = self.task_index
        #dataset_name = self.name.split('-')[0]

        with open(f'Datasets\FS_data\FG_graph.pkl', 'rb') as f: # 加载处理后的化学知识数据
            FG_graph = pickle.load(f)
        with open(f'Datasets\FS_data\Spectrum_graph.pkl', 'rb') as f: # 加载处理后的光谱数据
