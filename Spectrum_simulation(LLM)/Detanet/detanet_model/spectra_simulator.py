...
class nn_vib_analysis(torch.nn.Module):
    ...
    def forward(self,pos,z,edge_index=None,batch=None):
        ...
        Hi= self.model_Hi(pos=pos,z=z,batch=batch).to(self.device)
        Hij= self.model_Hij(pos=pos,z=z,edge_index=edge_index,batch=batch).to(self.device)
        dd= self.model_dd(pos=pos,z=z,batch=batch).to(self.device)
        dp= self.model_dp(pos=pos,z=z,batch=batch).to(self.device)
        if batch is None:
