...
class Edge_Attention(nn.Module):
    ...
    def __init__(self,num_radial,num_features,act,head=8):
        ...
        self.head=head
        self.actq = activations(act,num_features=num_features).to('cuda')
        self.actk = activations(act, num_features=num_features).to('cuda')
        self.actv = activations(act, num_features=2*num_features).to('cuda')
        self.acta = activations(act, num_features=2*num_features).to('cuda')
        self.softmax=activations('softmax')
        self.lq=nn.Linear(num_features,num_features).to('cuda')
        self.lk=nn.Linear(num_features,num_features).to('cuda')
        self.lv=nn.Linear(num_features,2*num_features).to('cuda')
        self.la=nn.Linear(2*num_features,2*num_features).to('cuda')
        self.lrbf=nn.Linear(num_radial,num_features,bias=False).to('cuda')
        self.lkrbf=nn.Linear(num_features,num_features,bias=False).to('cuda')
        self.lvrbf=nn.Linear(num_features,2*num_features,bias=False).to('cuda')
