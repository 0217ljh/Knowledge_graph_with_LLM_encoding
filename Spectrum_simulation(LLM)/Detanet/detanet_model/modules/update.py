...
class Tensorproduct_Attention(nn.Module):
    def __init__(self,num_features,irreps_T,act,):
        ...
        self.lq=o3.Linear(irreps_T,irreps_T, internal_weights=True,shared_weights=True).to('cuda')
        self.lk=o3.Linear(irreps_T,irreps_T, internal_weights=True,shared_weights=True).to('cuda')
        self.lv=o3.Linear(irreps_T,irreps_T, internal_weights=True,shared_weights=True).to('cuda')
        self.ls=nn.Linear(num_features,2*num_features).to('cuda')
        self.lvs=nn.Linear(num_features,num_features).to('cuda')
        ...
        self.tp1=o3.TensorProduct(irreps_T,irreps_T,irreps_scalar,instructions=intp1).to('cuda')
        self.tp2=o3.TensorProduct(irreps_scalar,irreps_T,irreps_T,instructions=intp2).to('cuda')
        self.softmax=activations('softmax').to('cuda')
        self.actlvs=activations(act, num_features=num_features).to('cuda')
...
class Update(nn.Module):
    def __init__(self,num_features,act,irreps_mout,irreps_T,dropout=0.0):
        ...
        self.actu=activations(act,num_features=num_features).to('cuda')
        self.drop=nn.Dropout(dropout).to('cuda')
        self.outt = o3.Linear(irreps_in=irreps_mout, irreps_out=irreps_T, internal_weights=True,
                                shared_weights=True).to('cuda')
        self.outs=nn.Linear(num_features,num_features).to('cuda')
