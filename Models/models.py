from Models.nn.layer.pyg import RGCNEdgeConv
...
class BinGraphModel(torch.nn.Module):
    ...
    def forward(self, g):
        ...
        # 获取模型的嵌入
        # emb = self.model(g)
        # print(f"g.true_nodes_mask shape: {g.true_nodes_mask.shape}")

        # # 对每个 emb 张量进行索引，emb 是一个 batch_size 的列表
        # class_emb_list = []

        # # 遍历每个批次中的嵌入
        # for b in range(len(emb)):  
        #     # 应用掩码，选择有效节点的嵌入，g.true_nodes_mask 是一个 [1479] 维度的布尔向量
        #     class_emb = emb[b][g.true_nodes_mask]  # 索引时应用掩码，选择有效节点的嵌入

        #     class_emb_list.append(class_emb)

        # # 将所有有效节点的嵌入堆叠成一个 tensor，形状为 [batch_size, valid_nodes, feature_dim]
        # # valid_nodes 是掩码中值为 True 的节点数量
        # class_embs = torch.stack(class_emb_list, dim=0)  # 堆叠所有批次的嵌入
        # print(class_embs.shape)
        # class_emb = class_emb.squeeze(0)  # 去除第一个维度，变成[40, 768]
        # print('after squ:',class_emb.shape)
        # # 使用 MLP 进行分类
        # res = self.mlp(class_embs)
